...
final int X_MAX     = (64+1)*X_DIST+1;        // x-axis lenght
...
{ 
  ...
  textFont(fontA, 20);
  fill(graphColor); 
  text("64-Channel Spectrum Analyser", 120, 40);
  textFont(fontA, 16);
  text("magnitude", 7, 20);  
  text("(8bit-value)", 7, 40);  
  text("--> channel (number i)", X_OFFSET, Y_WINDOW-Y_OFFSET/2);
  text(" frequency   f ( i ) = i * SAMPLE_RATE / FHT_N ", 200, Y_WINDOW-Y_OFFSET/2 );
} 
...
{ 
  ...
  port.buffer(65);                              // 1 start-byte + 128 data-bytes 
